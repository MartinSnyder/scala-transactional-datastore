...
abstract class AbstractBasicUpdateTest extends FunSpec {
  ...
  describe("Data Store") {
    ...
    it("can update records") {
      ...
      val retrievedOriginal = dataStore.withConnection(_.loadRecords(SampleCondition))
      assert(retrievedOriginal == Success(Seq()))

      // Query for the updated values.  They should exist in the store
      val retrievedUpdate = dataStore.withConnection(_.loadRecords(EqualsCondition("value", "First Update")))
